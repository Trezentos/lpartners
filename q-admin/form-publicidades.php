...
if($_POST && isset($_POST['submit'])) {
	switch($_POST['action']) {
		...
		if (empty($errors))
		{
			...
			$insert = $db->insert($tables['PUBLICIDADES'],$array_insert);
		...
		if (empty($errors))
		{
			...
			$update = $db->update($tables['PUBLICIDADES'],$array_update,array('id'=>$_POST['id']));
	...
	if($upload_temp[0] == 'true') {
		...
		$fileNewName = $id.'-banner-pub-'.rand(0,99);
		$image = new Image(TEMP.$fileName);
		$image->setPathToTempFiles(TEMP);
		$image->resize(1140, 290, "crop", "c", "c");
	...
	if($upload_temp[0] == 'true') {
		...
		$fileNewName_mobile = $id.'-banner-pub-mobile-'.rand(0,99);
		$image_mobile = new Image(TEMP.$fileName);
		$image_mobile->setPathToTempFiles(TEMP);
		$image_mobile->resize(600, 600, "crop", "c", "c");
...
if ($_POST && isset($_POST['deletar'])) {
	$query = $db->get_row("SELECT * FROM ".$tables['PUBLICIDADES']." WHERE id = '{$_POST["id"]}'");
	...
	$db->query("DELETE FROM ".$tables['PUBLICIDADES']." WHERE id='{$_POST['id']}'");

	header("Location: ".HTTP_GESTOR."list-publicidades.php?del=ok");
}

// DELETAR IMAGEM
if( is_numeric( $_GET["del_img"] ) && $_GET["del_img"] == "1" && $_GET["imagem"] == "1" )
{
	$query = $db->get_row("SELECT imagem FROM ".$tables['PUBLICIDADES']." WHERE id = '{$_GET["id"]}'");
	...
	$update = $db->update($tables['PUBLICIDADES'],$array_update,array('id'=>$_GET['id']));
...
// DELETAR IMAGEM MOBILE
if( is_numeric( $_GET["del_img"] ) && $_GET["del_img"] == "1" && $_GET["imagem_mobile"] == "1" ) {
	$query = $db->get_row("SELECT imagem_mobile FROM ".$tables['PUBLICIDADES']." WHERE id = '{$_GET["id"]}'");
...
if($id) $query = $db->get_row("SELECT * FROM ".$tables['PUBLICIDADES']." WHERE id='{$id}'");

// HEADERS
$_header['titulo'] = ($id?'Editar Banner - Publicidades':'Novo Banner - Publicidades');
...
<form name="form" id="form" action="" method="post" enctype="multipart/form-data" role="form">
	<div id="buttons">
		...
		<div class="pull-right">
			<a class="btn btn-sm btn-default hidden-xs" href="form-publicidades.php" title="Novo"><span class="glyphicon glyphicon-file"></span> Adicionar Novo</a>
			<a class="btn btn-sm btn-default" title="voltar" href="list-publicidades.php"><span class="glyphicon glyphicon-arrow-left"></span> Voltar</a>
		</div>
	</div>
	...
	<fieldset>
		...
		<div class="well">
			...
			<a class="btn btn-sm btn-danger" href="form-publicidades.php?id=<?php echo $id?>&imagem=1&del_img=1" title="Excluir"><span class="glyphicon glyphicon-trash"></span> Deletar Imagem</a>
			<?php } else { ?>
			<label for="imagem">Imagem</label>
			<input type="file" id="imagem" name="imagem">
			<p class="help-block">A imagem deve ter 1140x290px</p>
			<?php } ?>
		</div>
		...
		<div class="well">
			...
			<a class="btn btn-sm btn-danger" href="form-publicidades.php?id=<?php echo $id?>&imagem_mobile=1&del_img=1" title="Excluir"><span class="glyphicon glyphicon-trash"></span> Deletar Imagem Mobile</a>
			<?php } else { ?>

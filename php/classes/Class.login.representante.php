...
class Usuario {
     ...
     * @since v1.0
     */
    var $tabelaUsuarios = 'representantes';
     ...
     * @since v1.0
     */
    var $dados = array('id', 'nome', 'email', 'status');
     ...
     * @since v1.0
     */
    var $prefixoChaves = 'representante_';
    ...
    function validaUsuario($usuario, $senha) {
        ...
        $sql = "SELECT COUNT(*) AS total FROM ".$tables['REPRESENTANTES']." WHERE {$binary} `{$this->campos['usuario']}` = '{$usuario}' AND {$binary} `{$this->campos['senha']}` = '{$senha}' AND status = 1 LIMIT 1";

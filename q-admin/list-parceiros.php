...
// DELETAR REGISTRO
if($_GET['id']) {
	$db->query("DELETE FROM ".$tables['PARCEIROS']." WHERE id='{$_GET['id']}'");
...
$count = $db->get_var("SELECT COUNT(id) FROM ".$tables['PARCEIROS']);

// HEADERS
$_header['titulo'] = 'Parceiros';
...
<div id="buttons">
	<div class="text-right">
		<a class="btn btn-sm btn-default" href="form-parceiros.php" title="Novo"><span class="glyphicon glyphicon-file"></span> Adicionar Novo</a>
	</div>
</div>
...
if ($count>0) {
	...
	$query = $db->get_results("SELECT * FROM ".$tables['PARCEIROS']." ORDER BY id DESC $navbar->limit");
?>
		...
		<?php foreach($query as $rs) { ?>
			<tr>
				...
				<td class="text-center">
					<a class="btn btn-sm btn-primary" href="form-parceiros.php?id=<?php echo $rs->id; ?>" title="Editar" rel="tipsy"><span class="glyphicon glyphicon-edit"></span></a>
